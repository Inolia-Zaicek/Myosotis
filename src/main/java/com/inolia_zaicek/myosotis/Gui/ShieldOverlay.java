 ...
 */
public class ShieldOverlay implements IGuiOverlay {
    // 单例对象，用于注册渲染Overlay
    public static final ShieldOverlay instance = new ShieldOverlay();

    // 存放法力条纹理资源路径
    public static final ResourceLocation TEXTURE = new ResourceLocation("myosotis", "textures/gui/shield.png");
    ...
    public void render(ForgeGui gui, GuiGraphics guiHelper, float partialTick, int screenWidth, int screenHeight) {
        ...
        if (player != null && shouldShowManaBar(player)) {
            ...
            int nowShield = (int) MyUtil.getShield(player);
            int maxShield = (int) (player.getAttributeValue((Attribute) MyAttributesRegister.MaxShield.get())
                    * player.getAttributeValue((Attribute) Attributes.MAX_HEALTH));
            // 从配置读取法力条的偏移量（允许玩家调整显示位置）
            int configOffsetY = -10;
            int configOffsetX = 125;
            ...
            if (anchor != Anchor.XP || !(player.getJumpRidingScale() > 0.0F)) {
                ...
                guiHelper.blit(TEXTURE, barX, barY, spriteX, spriteY + 21, (int) (imageWidth * Math.min((double) nowShield / maxShield, 1.0)), 21);

                // 构造显示的法力值文本，例如 "34/100"
                String manaFraction = nowShield + "/" + maxShield;
    ...
    public static boolean shouldShowManaBar(Player player) {
        ...
        return !player.isSpectator() &&
                display != ShieldOverlay.Display.Never &&
                (
                        display == ShieldOverlay.Display.Always
                        || nowShield>0
                                /*
    ...
    private static int getBarX(Anchor anchor, int screenWidth) {
        if (anchor == ShieldOverlay.Anchor.XP) {
            // 如果锚点在经验（XP）条旁，放到屏幕正中央略偏左位置
            return screenWidth / 2 - 91 - 3;
        } else if (anchor != ShieldOverlay.Anchor.Hunger && anchor != ShieldOverlay.Anchor.Center) {
            // 如果锚点非饥饿条和中心，左右边距加载不同处理
            // 如果不是左上或左下，则放到右边距偏左的位置，否则放左边距偏右点的位置
            return anchor != ShieldOverlay.Anchor.TopLeft && anchor != ShieldOverlay.Anchor.BottomLeft ? screenWidth - 20 - 98 : 20;
        } else {
            // 饥饿条/中心锚点，放到屏幕宽度中间偏左或正中
            return screenWidth / 2 - 49 + (anchor == ShieldOverlay.Anchor.Center ? 0 : 50);
        }
    }
    ...
    private static int getBarY(Anchor anchor, int screenHeight, ForgeGui gui) {
        ...
        } else if (anchor == ShieldOverlay.Anchor.Hunger) {
            // 饥饿条锚点，结合rightHeight堆叠高度动态计算（避免遮挡）
            return screenHeight - (getAndIncrementRightHeight(gui) - 2) - 10;
        } else if (anchor == ShieldOverlay.Anchor.Center) {
            // 屏幕中心锚点，放在底部和经验条上方
            return screenHeight - 25 - 27 - 10;
        } else {
            // 左上/右上锚点靠屏幕顶端，否则放在底部偏下位置
            return anchor != ShieldOverlay.Anchor.TopLeft && anchor != ShieldOverlay.Anchor.TopRight ? screenHeight - 20 - 21 : 20;
        }
    }
    ...
    // 静态代码块初始化文字颜色为天蓝色
    static {
        TEXT_COLOR = ChatFormatting.YELLOW.getColor();
    }
